...
class BMWConnectedDriveSensor(BinarySensorDevice):
    ...
    def update(self):
        ...
        from bimmer_connected.state import LidState # to check if new LidState value is present
        vehicle_state = self._vehicle.state

        enum_list_lidstate = list(map(str, LidState))
        enum_list_lidstate_values = [e.value for e in LidState]
        _LOGGER.error("BMW Connected Drive -> No Error, just for testing -> Lidstates: %s", enum_list_lidstate)
        _LOGGER.error("BMW Connected Drive -> No Error, just for testing -> Lidstates values: %s", enum_list_lidstate_values)
